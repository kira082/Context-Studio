...
> **Design Philosophy:** Context Studio is designed as a **Plug & Play** component. The core No-Code platform (workflow engine, canvas) can function entirely without it for stateless automations. When Context Studio is enabled, it seamlessly augments the workflow engine via dedicated API contracts.
...
graph TB
    subgraph ClientLayer["Client Layer (React SPA)"]
        ...
        CSUI["Context Studio Dashboard<br/>(Memory Explorer & Curation)"]
    end
    ...
    subgraph StoragePlatform["Standard Storage"]
        PG_CORE["PostgreSQL<br/>(Blueprints, Users, Logs)"]
    end

    subgraph StorageMemory["Context Storage"]
        RD["Redis (Working Memory)"]
        PG_MEM["PostgreSQL (Conversations)"]
        QD["Qdrant (Vector / Episodic)"]
...
> **Purpose:** Details the internal micro-architecture of the Context Studio module itself, isolating how it processes, stores, and retrieves memory across multiple specialized databases.
...
graph TB
    ...
    subgraph MemoryStores["Isolated Memory Stores"]
        RD_WM["Redis Cluster<br/>(In-Memory Hash/TTL)"]
        PG_CM["PostgreSQL<br/>(Relational/JSONB)"]
        QD_EM["Qdrant<br/>(HNSW Vector Index)"]
        N4_KG["Neo4j<br/>(Property Graph)"]
    end
...
### 2.2 Data Flow — Single Agent Execution (Context Injection)
...
sequenceDiagram
    ...
    participant Stores as Redis/PG/Qdrant/Neo4j
    participant LLM as Agent LLM

    WFE->>MemAPI: Event (query, agentId, tenantId)
    
    Note over MemAPI,Stores: Phase 1 — Context Assembly (Read)
    MemAPI->>MemRead: getContext()
    par Parallel Retrieval
        MemRead->>Stores: Fetch Working, Conversational, Episodic, Semantic
    end
    Stores-->>MemRead: Raw Memory Candidates
    MemRead->>MemRead: Adaptive Rank & Token Budget Pack
    MemRead-->>WFE: Assembled Context Payload
    
    Note over WFE,LLM: Phase 2 — Execution
    WFE->>LLM: System Prompt + Memory Context + Query
    LLM-->>WFE: Response / Tool Calls
    
    Note over WFE,Stores: Phase 3 — Async Write-Back
    WFE--)MemAPI: writeBackEvent(interaction, response)
    MemAPI->>Stores: Extract Facts & Store (Fire-and-Forget)
```

---

## 3. Memory Layer Strategies
...
### 3.5 Procedural Memory (How-To / Behavioral Rules)
...
### 3.6 User Preference Memory (Personalization Profile)
...
### 3.7 Organizational Memory (Governed Knowledge)
...
### 4.1 Unified Memory Read Pipeline
...
### 4.2 Unified Memory Write-Back Pipeline
...
### 4.3 Memory Compaction Pipeline (Scheduled)
...
-- ============================================================
-- conversation_turns       (Section 3.2)
-- conversation_summaries   (Section 3.2)
-- semantic_facts           (Section 3.4)
-- procedural_rules         (Section 3.5)
-- user_preferences         (Section 3.6)
-- org_knowledge            (Section 3.7)
-- org_knowledge_history    (Section 3.7)
...
> [!NOTE]
> This architecture is informed by several state-of-the-art research papers. We adapted specific strategies to fit the needs of a production no-code agent platform, rather than explicitly citing them in the technical specifications.

| Adopted Strategy | Source Inspiration | How We Integrated It |
|:---|:---|:---|
| **Tiered Memory + Paging** | *MemGPT* | Modeled Working Memory as RAM and external stores (Qdrant/PG/Neo4j) as Disk, allowing the agent to explicitly page context. |
| **Mutable Core Memory Blocks** | *MemGPT* | Agents can read/write their own `Persona` and `Human` blocks in Working Memory via function calls. |
| **Heartbeat Mechanism** | *MemGPT* | After tool calls, the agent can request a "heartbeat" to continue processing without user input — enabling multi-step memory management. |
| **Semantic Density Gating** | *SimpleMem* | An LLM-based "semantic judge" evaluates the information gain of each dialogue segment, filtering out low-utility content before it enters Episodic Memory. |
| **Recursive Memory Consolidation** | *SimpleMem* | Our scheduled compaction pipeline recursively merges atomic memories into higher-level, abstract representations. |
| **Adaptive Query-Aware Retrieval** | *SimpleMem* | The system dynamically adjusts retrieval scope based on assessed query complexity. |
| **Multi-Level State Scoping** | *Mem0* | Memory retrieval is explicitly scoped by User, Session, or Agent levels. |
| **Graph-Based Entity Memory** | *Mem0* | New information is linked to existing entities in a Knowledge Graph (Neo4j) to capture relational structure. |
